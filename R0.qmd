# The Basic Reproduction Number {#sec-R0}
...
$$ R_0 = \tau \cdot \bar{c} \cdot d$${#eq-ro}
...
$$\lambda(t) = \frac{\tau \sum_i i Y_i(t)}{\sum_i i N_i(t)}$${#eq-force}

Integrate @eq-dyi ($dX_i/dt$) to get

$$X_i(t) = N_i \exp \left[-i \psi(t) \right],$${#eq-sum}
...
$$F = \left[ \frac{\partial F_i(x_0)}{\partial x_j} \right]$${#eq-F}

and

$$V = \left[ \frac{\partial V_i(x_0)}{\partial x_j} \right]$${#eq-V}
...
$$ R_0 = \frac{k \beta  \lambda }{\mu  (k +\mu ) (\gamma +\mu )} $${#eq-seir-ro}
...
@fig-gens shows a schematic representation of an epidemic. The zeroth generation of the epidemic is the index case---patient zero---indicated in red. The number of secondary infections generated by the case in generation zero is $R_0=3$. In the first generation (blue), $\mathcal{R}_1=6/3=2$. The second generation (cyan) has $\mathcal{R}_1=12/6=2$.

![Four generations of an epidemic.](figs/generations.png){#fig-gens}
...
$$  T_D = \frac{\log(2) V}{R_0 -1}.$${#eq-doubling}
...
$$  T_D = \frac{t_1 - t_0}{\log_2 \left(\frac{N_1}{N_0} \right)}$${#eq-double-2pts}
...
$$  \log(s_{\infty}) =  R_0 (s_{\infty}-1)$${#eq-final}
...
We will consider a model in which transmissibility and disease-induced mortality trade-off introduced by @vanbaalen_sabelis1995. This interaction is mediated by virulence, which we will take to be proportional to parasite burden or parasitemia. More copies of a virus (say) means that *conditional on contact* with an infected individual, the pathogen is more likely to be transmitted. However, more viral copies means the host is sicker---and potentially dead. Dead hosts do not transmit and very sick hosts are less likely to be up and interacting with susceptibles.
...
$$ R_0 = \frac{\beta}{\mu+\delta} > 1, $${#eq-ro1}
...
$$ \frac{d\beta(x)}{d\delta(x)} = \frac{\beta(x^*)}{\mu+\delta(x^*)}$${#eq-mvt}
